...
export default function ComandaScreen() {
  const { id, mesa_numero } = useLocalSearchParams();
  const [comanda, setComanda] = useState(null);
  const [loading, setLoading] = useState(true);

  useFocusEffect(useCallback(() => {
    api.get(`/comandas/${id}`).then(({ data }) => setComanda(data)).finally(() => setLoading(false));
  }, [id]));
  ...
  return (
    <View style={s.container}>
      <View style={s.header}>
        <TouchableOpacity onPress={() => router.back()}><Text style={s.back}>← Mesas</Text></TouchableOpacity>
        <Text style={s.titulo}>Mesa {mesa_numero}</Text>
      ...
      <View style={s.footer}>
        <TouchableOpacity style={s.btnAdd}
          onPress={() => router.push({ pathname: '/(garcom)/cardapio/[comandaId]', params: { comandaId: id, mesa_numero } })}>
...
const s = StyleSheet.create({
  ...
  back:      { color: '#aaa', fontSize: 14, marginRight: 12 },
  titulo:    { color: '#fff', fontWeight: '700', fontSize: 18, flex: 1 },
  total:     { color: '#2ecc71', fontWeight: '700', fontSize: 16 },
